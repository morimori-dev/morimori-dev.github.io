...
- **業務経験**:
  - **インフラ運用保守 8 年** — **大規模案件中心**、**Windows パッチ配信** の設計・運用も担当
  - **脆弱性診断 + プラットフォーム診断**（**2025-07 〜** / 攻撃側のキャリア開始）
- **保有資格**:
  - **情報処理安全確保支援士 (RISS)**
...
> **率直な所感**: コマンドを実行したら何が起きるか、OS の最低限の知識はわかっていた、という感じ。8 年のインフラ運用で叩き込まれた「Linux/Windows のどこに何があるか」「サービスがどう動くか」の感覚はそのまま効いた。
>
> 一方で **攻撃側のマインドセット**（権限境界をどう越えるか、認証情報をどう連鎖させるか）はゼロからで、ここに最も時間を使った。同じインフラ系出身の方には、**運用知識は確実に武器になる**と伝えたい。

### なぜ OSCP を受けようと思ったか

正直、最初の衝動はよく覚えていない。多分ツイッターで「OSCP を持っているといい感じになるらしい」と知ったんだと思う。それまで防御側の資格 (RISS / AWS Security 等) は取ってきたけれど、**「攻撃側の視点を持っている」と公的に証明できる資格** は手元になかった。インフラ運用と脆弱性診断を繋ぐためにも、OSCP は外せないと思った。

ちなみに取る前は「OSCP が取れたら道が開けて世界が変わるんだろうな」と漠然と思っていた。実際に取ってみると、世界は全然変わらなかった。次のハードル（OSEP、CRTO、CPTS…）が見えてきて、目指す場所が増えて、新しい世界が広がっただけだった。結局、資格は「ゴール」じゃなくて「次のスタート地点」なんだと今は思っている。
...
試験環境の快適さも意外と合否に効く。2回目では以下を用意した。
...
- **Webカメラ** — OffSec の試験規約でプロクタリングが必要。事前に映り・角度・照明を確認しておくと、試験開始直前の余計なストレスがゼロになる
...
| 経過時間 | 進捗 |
|---|---|
| 0h | 試験開始、全マシン nmap 同時投入 |
| ~0.5h | AD 環境侵入 → 初期 foothold |
| ~2h | AD user.txt（10点）|
| ~4h | AD pivot → DC 侵入 → AD 完全制圧（**40点**）|
| ~4.5h | 軽食 + Standalone1 列挙開始 |
| ~6.5h | Standalone1 user.txt（10点）|
| ~8h | Standalone1 root.txt（**+10点 / 計 60点**）|
| ~8.5h | Standalone2 列挙開始 |
| ~10.5h | Standalone2 user.txt（10点）|
| ~12h | Standalone2 root.txt（**+10点 / 計 80点 = 合格ライン超え**）|
| ~13.5h | Standalone3 触るも深追いせず、スクリーンショットと残コマンド整理に切り替え |
| ~17h | 仮眠 4h |
| ~21h | スクショ抜け確認、必要分の追加取得 |
| 24h | 試験終了 |
...
5. **Flameshot でスクショ管理を統一**: [Flameshot](https://flameshot.org/) にショートカットを割り当て、撮影時にそのまま枠やハイライトを追加できる状態を作った。撮る→貼るの動作を反射化しておいたことで、証跡整理で手が止まることがなかった
...
- **ホワイトボードを準備したこと**: AD の列挙結果・ピボット経路・権限委任の関係・ユーザー昇格の手筋を図に描くことで、頭の中だけでは整理しきれない情報が視覚化できた。特に「どのユーザーからどこへ横展開できるか」「どこでピボットを張るか」をボードに書き出すと、詰まった時に見渡して次の手が思い浮かびやすくなった
- **過去 writeup のダッシュボードを作ったこと**: 積み上げた writeup の検索性を上げるため、使用ツール・脆弱性の種類・CMS・権限昇格手法などのタグで横断検索できるダッシュボードを自作した。「以前 LFI から RCE に繋げたのどのマシンだっけ」「Potato 系を使ったのいくつあったか」といった問いに即座に答えられる状態を作ることで、試験本番でも「このパターン、あのマシンで見た」と手筋を引き出しやすくなった
- **PEN-200 は Skylark 以外全モジュール完走したこと**: コース単体では不足とはいえ、基礎の土台はここで作るのが一番効率がいい。章末エクササイズも含めてひと通りやり切ってから外部マシンに移った
- **英語は DeepL に頼り切ったこと**: コースは全編英語で最初は不安しかなかったが、DeepL があればほぼ問題なかった。専門用語が稀にズレるので原文と照合する習慣だけつけておくと安心
...
- **BloodHound とにらめっこ**: 取れた認証情報・セッション・ACL を全部食わせて、エッジを目で追う。攻撃パスは BloodHound のグラフが教えてくれる。ただし BloodHound は稀に嘘をつくので、怪しいエッジは PowerView.ps1 でも必ず裏取りする
...
今は **AI を活用したペネトレーションテスト** と **C2 フレームワークの運用**に強い興味があります。LLM を偵察・列挙・脆弱性分析に組み込む「AI-Augmented Pentest」の領域は実用フェーズに入りつつあり、攻撃オペレーターとして乗り遅れたくないと思っています。C2 側は Cobalt Strike / Havoc / Sliver などのフレームワークを深掘りして、**OPSEC を意識した長期侵害シナリオ**を体で覚えたいと考えています。
