...
date: 2026-03-06
permalink: /ja/posts/tech-windows-privesc-summary/
legacy_permalink: /posts/tech-windows-privesc-summary-ja/
...
mermaid: true
content_lang: ja
alt_en: /en/posts/tech-windows-privesc-summary/
---
...
**[→ 全 Writeup](/en/posts/pg-bratarina/)**
...
**観測事例:** [THM - Alfred](/en/posts/thm-alfred/) — Jenkins → SeImpersonate → PrintSpoofer → SYSTEM
...
**観測事例:** [HTB - Active](/en/posts/htb-active/), [THM - Corp](/en/posts/thm-corp/)
...
**観測事例:** [THM - Windows PrivEsc Arena](/en/posts/thm-windows-privesc-arena/)
...
**観測事例:** [THM - Windows PrivEsc](/en/posts/thm-windows-privesc/)
...
**観測事例:** [HTB - Active](/en/posts/htb-active/)
...
**観測事例:** [HTB - Forest](/en/posts/htb-forest/)
...
**観測事例:** [THM - Corp](/en/posts/thm-corp/)
...
|--------|-----------|----------|---------|-------|
| **Craft2** | Windows 10 | Bad-ODF NTLM → Web シェル | CVE-2020-1337（WerTrigger + DLL） | [→](/en/posts/pg-craft2/) |
| **Apex** | Linux（Ubuntu） | OpenEMR LFI（CVE-2018-17179） | sqlconf.php → MySQL 資格情報 | [→](/en/posts/pg-apex/) |
| **Bratarina** | Linux（Samba） | OpenSMTPD RCE（CVE-2020-7247） | エクスプロイトで直接 root | [→](/en/posts/pg-bratarina/) |
| **Clue** | Linux（Debian） | Cassandra Web LFI → FreeSWITCH RCE | sudo → root として LFI 再利用 | [→](/en/posts/pg-clue/) |
...
| **Forest** | LDAP ユーザー列挙 | AS-REP Roast → BloodHound → DCSync | [→](/en/posts/htb-forest/) |
| **Fluffy** | （Writeup 参照） | （Writeup 参照） | [→](/en/posts/htb-fluffy/) |
| **Legacy** | （Writeup 参照） | （Writeup 参照） | [→](/en/posts/htb-legacy/) |

### TryHackMe Windows（主要マシン）

| マシン | 侵入経路 | 権限昇格 | リンク |
|--------|----------|---------|-------|
| **Windows PrivEsc** | ローカルシェル | 書き込み可スクリプト + SYSTEM スケジューラー | [→](/en/posts/thm-windows-privesc/) |
| **Windows PrivEsc Arena** | RDP | サービス設定不備・AlwaysInstallElevated・引用符なしパス | [→](/en/posts/thm-windows-privesc-arena/) |
| **Alfred** | Jenkins デフォルト認証 | SeImpersonate → PrintSpoofer | [→](/en/posts/thm-alfred/) |
| **Corp** | ローカル RDP | Kerberoasting + Unattend.xml | [→](/en/posts/thm-corp/) |
| **Retro** | WordPress 資格情報 → RDP | カーネルエクスプロイト | [→](/en/posts/thm-retro/) |
| **Steel Mountain** | Rejetto HFS CVE | サービス設定悪用 | [→](/en/posts/thm-steel-mountain/) |
| **HackPark** | Web ブルートフォース | （Writeup 参照） | [→](/en/posts/thm-hackpark/) |
| **Blaster** | （Writeup 参照） | （Writeup 参照） | [→](/en/posts/thm-blaster/) |
| **Holo** | （Writeup 参照） | （Writeup 参照） | [→](/en/posts/thm-holo/) |
| **Stealth** | （Writeup 参照） | （Writeup 参照） | [→](/en/posts/thm-stealth/) |
| **Attacking Kerberos** | Kerberos 攻撃 | AD ラボ | [→](/en/posts/thm-attacking-kerberos/) |
| **Attacktive Directory** | AD ラボ | AD ラボ | [→](/en/posts/thm-attacktive-directory/) |

### 関連 TechBlog 記事

| 記事 | リンク |
|------|-------|
| Windows Potato PrivEsc ガイド（GodPotato ～ Hot Potato 全解説） | [→](/en/posts/tech-windows-potato-privesc/) |
| PsExec ラテラルムーブメント | [→](/en/posts/tech-psexec-lateral-movement/) |
| NTLM リレー（ntlmrelayx） | [→](/en/posts/tech-ntlmrelayx-attack-guide/) |
| Kerberoasting（GetUserSPNs 詳解） | [→](/en/posts/tech-getuserspns-kerberoasting/) |
| RBCD 攻撃 | [→](/en/posts/tech-rbcd-attack-guide/) |
