...
| Analyst | morimori |
| Primary Entry Vector | LimeSurvey 5.3.24 RCE (CVE-2021-44967) |
| Privilege Escalation Path | Credential discovery -> `su` user switch -> `sudo` abuse -> root |
...
| Attack Surface | `22/tcp` (SSH), `80/tcp` (Apache HTTP), virtual host `customers-survey.marketing.pg`, LimeSurvey admin panel |
